...
// ---- AUTH ----
// Supabase client is created in init() after fetching the public config from
// the backend, so no keys are hardcoded in this file.
let sb = null;
let session = null;
...
async function init() {
    // Fetch the public Supabase config from the backend (keys live in .env).
    try {
        const cfg = await (await fetch(`${API_BASE}/config`)).json();
        if (window.supabase && cfg.url && cfg.key) {
            sb = window.supabase.createClient(cfg.url, cfg.key);
        }
    } catch (e) { /* auth stays disabled if config can't load */ }
